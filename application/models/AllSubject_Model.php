...
class AllSubject_Model extends CI_Model {
    ...
    public function getSubjectWithCountSection($semester_id)
    {
        ...
        (select IFNULL(SUM(case when LENGTH(sec.Section_id) < 3 then 1 else 0 end),0) from Section sec where sec.Semester_ID = ? AND sec.Subject_id = sub.Subject_id) as GroupNomal ,
        (select IFNULL(SUM(case when LENGTH(sec.Section_id) >= 3 then 1 else 0 end),0) from Section sec where sec.Semester_ID = ? AND sec.Subject_id = sub.Subject_id) as GroupSpecial
        FROM `Subject` sub";
